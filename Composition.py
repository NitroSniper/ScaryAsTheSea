...
class PolygonOverviewObject(object):
    def __init__(self, TupleOfPolygonObject):
        self.argsDict, self.polygons = {}, {}
        
        for i, argsAsDict in enumerate(TupleOfPolygonObject):
            self.argsDict[i] = argsAsDict
            self.polgons[i] = PolygonInformationObject(**argsAsDict)
        self.rotation = 0
        
        # args = (self.verticesNum, self.radius, self.color, self.rotation, self.IMGAlpha)
        # self.image = GFXDrawShapes()
        #self.image = GFXDrawShapes. #Something
